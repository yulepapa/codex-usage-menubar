...
- 원본 Codex 윤곽선 아이콘과 남은 사용량 백분율을 메뉴 막대에 바로 표시
- 밝은 메뉴 막대와 어두운 메뉴 막대에 맞춰 아이콘 색상을 자동 조정
...
메뉴 막대에 원본 Codex 윤곽선 아이콘과 남은 사용량 백분율이 나란히 표시됩니다. 아이콘 색상은 메뉴 막대의 밝기에 맞춰 자동으로 조정됩니다. 최초 조회 중에는 아이콘 옆에 `…`가 표시되며, `!`는 사용량을 불러오지 못했음을, `?`는 표시할 사용량 구간이 없음을 뜻합니다. 항목을 클릭하면 다음 정보를 확인할 수 있습니다.
...
Hidden Bar, Bartender 또는 다른 메뉴 막대 관리 앱을 사용하면 새 항목이 처음부터 숨김 영역에 들어갈 수 있습니다. 관리 앱의 숨김 영역을 펼친 뒤 `Command` 키를 누른 채 Codex 윤곽선 아이콘을 항상 표시되는 영역으로 드래그하세요.
